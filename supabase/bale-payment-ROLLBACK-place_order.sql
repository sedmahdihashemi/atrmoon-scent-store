-- =====================================================================
-- بازگرداندن place_order به نسخه‌ی قبل از پرداخت بله (۹ پارامتر، بدون
-- payment_method). این را فقط اجرا کنید اگر می‌خواهید تغییرات
-- bale-payment-02-schema.sql را برای تابع place_order لغو کنید.
--
-- بدنه‌ی تابع پایین دقیقاً همان چیزی است که در
-- supabase/migrations/20260516185449_fb38cfb6-5c4a-482b-907a-23760e52b999.sql
-- بود (کپی مستقیم، نه از حافظه) — یعنی همان تابعی که همین الان (قبل از
-- هر migration پرداخت بله) روی دیتابیس شما در حال اجراست.
--
-- توجه: این فایل فقط place_order را برمی‌گرداند. ستون‌های جدید روی
-- orders، جدول bale_payment_events، و expire_pending_bale_orders را
-- دست نمی‌زند (بی‌خطرند و استفاده‌نشده می‌مانند اگر این فایل را اجرا کنید).
-- =====================================================================

BEGIN;

DROP FUNCTION IF EXISTS public.place_order(uuid,text,text,text,text,text,text,text,text,text);
